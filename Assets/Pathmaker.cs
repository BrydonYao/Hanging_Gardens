...
public class Pathmaker : MonoBehaviour
{
    ...
    void Update()
    {
        if (counter < 100)
        {
            ...
            else if (rnd > 0.25f && rnd < 0.8f)
            {
                transform.Rotate(Vector2.right * -90);
            }
            else if (rnd > 0.5f && rnd < 0.99f)
            {
                transform.position += transform.forward * speed;
                //Object.Instantiate(floorPrefab, transform.position, Quaternion.Euler(0,90,0));
                Object.Instantiate(floorPrefab, transform.position, transform.rotation);
                counter++;
                tiles++;
                if (tiles > 200)
                {
